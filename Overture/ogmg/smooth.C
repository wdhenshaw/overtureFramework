...
//---------------------------------------------------------------------------------------------
{
  // printF("Entering applyOgesSmoother: level=%i grid=%i\n",level,grid);
    ...
    IntegerArray & ogesActiveGrids = parameters.dbase.get<IntegerArray>("ogesActiveGrids");
    if( grid!=ogesActiveGrids(0) )  // This smooth does all grids in the ogesActiveGrids list at once.
        return; 
    ...
    if( ogesSmoother==NULL )
    {
        ...
        for( int l=0; l<mgcg.numberOfMultigridLevels()-1; l++ )
        {
            ...
            ogesSmoother[l].setGridsToUse(ogesActiveGrids);
      ...
      // printF("***applyOgesSmoother:before finishBC coeff.getIsACoefficientMatrix()=%i\n",coeff.getIsACoefficientMatrix());
            ...
            op.finishBoundaryConditions(coeff,bcParams,nullRange,ogesActiveGrids);
