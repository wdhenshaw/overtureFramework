...
// ================================================================================
/// \brief Return x,y,z min-max bounds of a component grid
/// \param xab[axis][side] : 
///      xMin[0] = min x-coordinate, xMax[0] = max-x coordinate
///      xMin[1] = min y-coordinate, xMax[1] = max-y coordinate
///      xMin[2] = min z-coordinate, xMax[2] = max-z coordinate
// ================================================================================
void GridStatistics::
    ...
    if( mg.getGridType()==GenericGrid::structuredGrid )
    {
      ...
      // This macro defines the grid points for rectangular grids:
            ...
            if( maskOption==ignoreMask )
            {
                for( int axis=0; axis<3; axis++ )
                {
                    xMin[axis] = xab[0][axis];
                    xMax[axis] = xab[1][axis];
                }
            }
            else
            {
                ...
                {
                  if(
                            (maskOption==positiveMask && maskLocal(i1,i2,i3)>0  ) ||
                            (maskOption==nonZeroMask  && maskLocal(i1,i2,i3)!=0 )
                        )
                    {
                        for( int axis=0; axis<rangeDimension; axis++ )
                        {
                            xMin[axis] = min( xMin[axis],XC(iv,axis) );
                            xMax[axis] = max( xMax[axis],XC(iv,axis) );
                        }
                    }
                }
            }
...
#define x(i0,i1,i2,i3) vertexp[i0+vertexDim0*(i1+vertexDim1*(i2+vertexDim2*(i3)))]
            ...
            FOR_3D(i1,i2,i3,I1,I2,I3)
            {
                    ...
                    )
                {
                    for( int axis=0; axis<rangeDimension; axis++ )
                    {
                        xMin[axis] = min( xMin[axis],x(i1,i2,i3,axis) );
                        xMax[axis] = max( xMax[axis],x(i1,i2,i3,axis) );
                    }
                }
            }
        ...
        for( int axis=0; axis<rangeDimension; axis++ )
        {
            xMin[axis]=ParallelUtility::getMinValue(xMin[axis]);
            xMax[axis]=ParallelUtility::getMaxValue(xMax[axis]);

        }     
    }
    else // unstructured grid
    {
        printF("GridStatistics::getGridBounds: WARNING: Not implemented yet for unstructured grids\n");
...
// ================================================================================
// Return x,y,z min-max bounds of a grid collection
/// \param xab[axis][side] : 
///      xab[0][0] = min x-coordinate, xab[0][1] = max-x coordinate
///      xab[1][0] = min y-coordinate, xab[0][1] = max-y coordinate
///      xab[2][0] = min z-coordinate, xab[0][1] = max-z coordinate
// ================================================================================
void GridStatistics::
getGridCoordinateBounds(GridCollection & gc, Real xMin[3], Real xMax[3], MaskOptionEnum maskOption /* =positiveMask */ )
{
    ...
    Real xMing[3], xMaxg[3]; // holds component grid min/max
    for( int grid=0; grid<gc.numberOfComponentGrids(); grid++ )
    {
        MappedGrid & mg = gc[grid];
        getGridCoordinateBounds(mg, xMing, xMaxg, maskOption );

        for( int axis=0; axis<3; axis++ )
        {
            xMin[axis] = min(xMin[axis],xMing[axis]);
            xMax[axis] = max(xMax[axis],xMaxg[axis]);
        }
    }
}
...
// =================================================================================================
{
    ...
    getGridCoordinateBounds( mg,xMin,xMax );
...
#define x(i0,i1,i2,i3) vertexp[i0+vertexDim0*(i1+vertexDim1*(i2+vertexDim2*(i3)))]
        ...
        // ************ 3D ***********************
            ...
            {
                fPrintF(file,
                                " xMin=[%9.2e,%9.2e], xMax=[%9.2e,%9.2e] (mask>0)\n"
            ...
            {
                fPrintF(file,
                                " xMin=[%9.2e,%9.2e,%9.2e], xMax=[%9.2e,%9.2e,%9.2e] (mask>0)\n"
