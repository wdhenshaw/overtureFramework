...
#
# For scattering
# ogen -noplot flattenedTorusGrid.cmd -prefix=flatTorusGridALongX -angle=90 -xa=-0.5 -xb=1.5 -ya=-1 -yb=1 -za=-1 -zb=1 -blf=2 -interp=e -factor=2
# ogen -noplot flattenedTorusGrid.cmd -prefix=flatTorusGridAlongX -angle=90 -xa=-0.5 -xb=1.5 -ya=-1 -yb=1 -za=-1 -zb=1 -blf=2 -interp=e -order=4 -numGhost=3 -factor=4
#
$prefix="flattendTorusGrid";
...
$pi = 4.*atan2(1.,1.);
#
# NOTE: do NOT extend the grid too far since the core region needs some background grid in it.
# nr = number of lines in normal directions to boundaries
$nr = max( 6 + $ng + 2*($order-2), 2**($ml) );
$nr = intmg( $nr );
#
